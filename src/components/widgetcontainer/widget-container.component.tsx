...
import { TWidget } from '../../common/type.def';
...
const WidgetContainer: React.FC<WidgetContainerProps> = (props) => {
    ...
    return (
        <div className="widget-container" style={{ height: '100%' }}>
            {editMode ? (
                <div style={{ width: '100%', height: '51px', backgroundColor: theme.palette.grey['300'] }}>
                    {widget === null ? (
                        <Grid container direction="row-reverse" spacing={1}>
                            <IconButton
                                onClick={() => dispatch(openAddWidgetModal(columnID, rowIndex))}
                                color="primary"
                            >
                        ...
                        <Grid container direction="row-reverse" spacing={1}>
                            <IconButton
                                onClick={() => dispatch(deleteWidget(widget !== null ? widget.id : 'DELETE ME'))}
                                color="error"
                            >
